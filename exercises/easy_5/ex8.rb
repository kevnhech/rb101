# Write a method that takes an Array of Integers between 0 and 19, and returns an Array of those Integers sorted based on the English words for each number

# input: array of integers between 0 and 19
# output: array of those integers sorted based on the english words for each number

# initialize constant `NUMBERS` with an array of element strings `zero` through `nineteen`
# initialize hash `hsh`
# initialize array `arr`
# iterate through `NUMBERS`
#   assign each element as a key for `hsh`, then assign those keys to numbers `0` through `19`
# end the block
# sort `hsh` based on the keys
# iterate through `hsh`
#   append every value of `hsh` to `arr`
# end the block
# return `arr`

NUMBERS = %w(zero one two three four five six seven eight nine ten eleven twelve thirteen fourteen fifteen sixteen seventeen eighteen nineteen)

def alphabetic_number_sort(array)
  array.sort_by { |number| NUMBERS[number] }
end

p alphabetic_number_sort((0..19).to_a) == [
  8, 18, 11, 15, 5, 4, 14, 9, 19, 1, 7, 17,
  6, 16, 10, 13, 3, 12, 2, 0
]